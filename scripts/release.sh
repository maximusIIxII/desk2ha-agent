...
#
# For single-repo publish without the full orchestrator flow, use:
#   ./scripts/predeploy.sh && ./scripts/publish.sh [bump]
#
...
if [[ -f "$ORCHESTRATOR" ]]; then
    echo "Delegating to orchestrator: $ORCHESTRATOR --agent $*"
    exec python3 "$ORCHESTRATOR" --agent "$@"
fi
